...
WITH avg_permit_time AS (
  ...
  FROM [SolarData2]
  GROUP BY [ahj],[state]), 
  --Group by AHJ and State are required for unique key later in the joins.
  --There can be several AHJ's with same names. (i.e. Clark County NV & WI)
...
--Full outer join due to abundance of supplimentary data that is still relevant to scope--
